// Same-origin module worker for @ffmpeg/ffmpeg v0.12.10 (MIT).
// Derived from ffmpegwasm/ffmpeg.wasm tag v0.12.10 packages/ffmpeg/src/worker.ts.
...
self.onmessage = async ({ data: { id, type, data: payload } }) => {
  ...
  try {
    ...
    self.postMessage({ id, type: FFMessageType.ERROR, data: e instanceof Error ? e.toString() : String(e) });
    return;
  }
