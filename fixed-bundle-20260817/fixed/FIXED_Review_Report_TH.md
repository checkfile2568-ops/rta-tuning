...
| Spreadsheet production | `[PRODUCTION_SPREADSHEET_ID]` ตามข้อมูลที่ได้รับ |
| Spreadsheet staging | `[STAGING_SPREADSHEET_ID]` |
| Web App | anonymous access, execute as deploying user |
| WEB_ADMIN_KEY | ต้องคงค่า `[WEB_ADMIN_KEY_FROM_EXISTING_CONFIG]` |
...
| Dashboard | key ผ่านหน้าแต่ command ไม่ถูก guard | key `[WEB_ADMIN_KEY_FROM_EXISTING_CONFIG]`, theme และ flow เดิมเป็นหลัก |
...
5. **รักษา `WEB_ADMIN_KEY=[WEB_ADMIN_KEY_FROM_EXISTING_CONFIG]`** แต่ไม่ hardcode key ใน GitHub และไม่เก็บไว้ใน source สาธารณะ
...
ระบบเดิมยังมีฐานการทำงานที่นำไปต่อได้ และไม่จำเป็นต้องรื้อ schema หรือรูปแบบแจ้งเตือน. แต่ควรแก้ P0 ก่อน ได้แก่ batch webhook, signature, type guard, server-side authorization และ debug data exposure. fixed bundle ที่จัดทำไว้เป็นชุดสำหรับ staging/ตรวจรับ ไม่ใช่การ deploy production อัตโนมัติ และไม่มีการเปลี่ยนข้อมูลจริงหรือ `WEB_ADMIN_KEY=[WEB_ADMIN_KEY_FROM_EXISTING_CONFIG]`
